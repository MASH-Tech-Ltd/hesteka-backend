...
const run = async () => {
  try {
    ...
    const resLogs = await securityLogModel.updateMany(
      {},
      { $set: { resetStrike: true } },
    );
    console.log(
      `Updated ${resLogs.modifiedCount} security logs to resetStrike = true.`,
    );

    // Remove any blocked IPs created during testing
    const resBlocked = await blockedIpModel.deleteMany({
      ip: "103.216.58.105",
    });
    console.log(
